...
def bio_retrieval_module(input_folder_path, input_type, model_folder_path, conversion_factor, show_message):
    ...
    # __________________________Construct variables______________________________
    # Create output path
    ...
    # __________________________Split processing by file type_______________________
    ...
    if input_type == 'CHIME netCDF':
        ...
        try:
            ...
            with open(logfile_path, 'w') as fileID:
                fileID.write('FAIL: Wrong number of inputs or error loading input image.'
        ...
        for i in range(num_images):
            ...
            try:
                ...
                with open(logfile_path, 'w') as fileID:
                    fileID.write('FAIL: Missing complementary files for CHIME image.'
                                 ' Consider checking Input Path/File \n')
                    fileID.write(f'Input Path: {input_path} \n')
                return 1
            except Exception as e:
                print("Error:", e)
                show_message("An unexpected error occurred. While copying complementary file.")
                with open(logfile_path, 'w') as fileID:
                    fileID.write('FAIL: An unexpected error occurred.'
                                 ' Consider checking Input Path/File \n')
                    fileID.write(f'Input Path: {input_path} \n')
                return 1
        ...
        try:
            ...
            with open(logfile_path, 'w') as fileID:
                fileID.write('FAIL: Wrong number of inputs or error loading input file.'
                             ' Consider checking Input Path/File \n')
                fileID.write(f'Input Path: {input_path} \n')
            return 1
        ...
        input_files = [os.path.join(input_path, hdr_files[i]) for i in range(num_images)]
        input_names = [os.path.splitext(os.path.basename(hdr_files[i]))[0] for i in range(num_images)]

        # Generate processing time
        proces_time = datetime.now().strftime('%Y%m%dT%H%M%SZ')
        # Collecting the output filenames here
        l2b_output_files = [os.path.join(output_path, f"{input_names[i]}_{proces_time}") for i in range(num_images)]
    ...
    for i in range(num_images):
        ...
        if i == 0:
            ...
            if flag_out:
                ...
                show_message('Output folder already exists. Folder was overwritten.')
                log_file_id.log_message('Output folder already exists. Folder was overwritten.\n')
            else:
                print('Output folder does not exist. Folder was created.')
                show_message('Output folder does not exist. Folder was created.')
                log_file_id.log_message('Output folder does not exist. Folder was created.\n')
        ...
        if return_value == 1:  # There was an error
            ...
            export_value = retrieval.export_retrieval()
